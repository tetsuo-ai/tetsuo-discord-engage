    ...
    async def get_metrics(self):
        ...
        try:
            ...
            try:
                ...
                if text_element:
                    print("Found text: How do you feel about TETSUO/SOL today?")
                    # Get the percentage div and just grab its text
                    percent_element = await page.query_selector("div.bg-buy.rounded-l")
                    if percent_element:
                        text = await percent_element.inner_text()
                        print(f"Found percentage text: {text}")
                        try:
                            value = float(text.strip('%'))
                            print(f"Found positive sentiment: {value}%")
                            return value
                        except ValueError:
